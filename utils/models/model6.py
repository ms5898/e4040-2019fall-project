...
def SVHNmodel(input_x, input_y, label_length, is_training=False, drop_rate=0.5):
    ...
    with tf.variable_scope('hidden_layer_1'):
        conv1 = tf.layers.conv2d(inputs=input_x, filters=128, kernel_size=[5, 5], padding='same')
    ...
    with tf.variable_scope('hidden_layer_2'):
        conv2 = tf.layers.conv2d(inputs=layer1, filters=160, kernel_size=[5, 5], padding='same')
    ...
    with tf.variable_scope('hidden_layer_3'):
        conv3 = tf.layers.conv2d(inputs=layer2, filters=192, kernel_size=[5, 5], padding='same')
    ...
    with tf.variable_scope('hidden_layer_4'):
        conv4 = tf.layers.conv2d(inputs=layer3, filters=192, kernel_size=[5, 5], padding='same')
        norm4 = tf.layers.batch_normalization(conv4)
        act4 = tf.nn.relu(norm4)
        pool4 = tf.layers.max_pooling2d(inputs=act4, pool_size=[2,2], strides=2, padding='same')
    ...
    flatten = tf.reshape(layer4, [-1, 9408]) #3072 get from the paper
    with tf.variable_scope('densely_hidden_layer_1'):
        dense2 = tf.layers.dense(inputs=flatten, units= 3072, activation=tf.nn.relu)
        layer10 = dense2
