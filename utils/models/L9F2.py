...
def SVHNmodel(input_x, input_y, label_length, is_training=False, drop_rate=0.5):
    ...
    with tf.variable_scope('hidden_layer_6'):
        conv6 = tf.layers.conv2d(inputs=layer5, filters=62, kernel_size=[5, 5], padding='same')
    ...
    flatten = tf.reshape(layer6, [-1, 3038]) #3072 get from the paper
...
def training(x_train, train_label, train_label_length,
    ...
    with tf.Session() as sess:
        ...
        writer = tf.summary.FileWriter("log92F/{}".format(cur_model_name), sess.graph)
            ...
            for itr in range(iters):
                ...
                if iter_total % 400 == 0:
                    ...
                    if valid_acc > best_acc:
                        ...
                        best_acc = valid_acc
                        saver.save(sess, 'model92F/{}'.format(cur_model_name))
