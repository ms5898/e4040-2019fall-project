...
def SVHNmodel(input_x, input_y, label_length, is_training=False, drop_rate=0.5):
    ...
    with tf.variable_scope('hidden_layer_2'):
        conv2 = tf.layers.conv2d(inputs=layer1, filters=192, strides=2, kernel_size=[5, 5], padding='same')
    ...
    flatten = tf.reshape(layer2, [-1, 4*4*192]) #3072 get from the paper
...
def training(x_train, train_label, train_label_length,
    ...
    with tf.Session() as sess:
        ...
        writer = tf.summary.FileWriter("log4/{}".format(cur_model_name), sess.graph)
            ...
            for itr in range(iters):
                ...
                if iter_total % 300 == 0:
                    ...
                    if valid_acc > best_acc:
                        ...
                        best_acc = valid_acc
                        saver.save(sess, 'model4/{}'.format(cur_model_name))
