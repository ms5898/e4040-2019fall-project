...
def SVHNmodel(input_x, input_y, label_length, is_training=False, drop_rate=0.5):
    ...
    with tf.variable_scope('hidden_layer_1'):
        conv1 = tf.layers.conv2d(inputs=input_x, filters=160, strides=2, kernel_size=[5, 5], padding='same')
    ...
    with tf.variable_scope('hidden_layer_2'):
        conv2 = tf.layers.conv2d(inputs=layer1, filters=192, strides=1, kernel_size=[5, 5], padding='same')
        norm2 = tf.layers.batch_normalization(conv2)
        act2 = tf.nn.relu(norm2)
        pool2 = tf.layers.max_pooling2d(inputs=act2, pool_size=[2, 2], strides=2, padding='same')
        drop2 = tf.layers.dropout(pool2, rate=drop_rate)
        layer2 = drop2

    with tf.variable_scope('hidden_layer_3'):
        conv3 = tf.layers.conv2d(inputs=layer2, filters=960, kernel_size=[5, 5], padding='same')
    ...
    flatten = tf.reshape(layer3, [-1, 4*4*960]) #3072 get from the paper
...
def training(x_train, train_label, train_label_length,
    ...
    with tf.Session() as sess:
        ...
        writer = tf.summary.FileWriter("log5/{}".format(cur_model_name), sess.graph)
            ...
            for itr in range(iters):
                ...
                if iter_total % 300 == 0:
                    ...
                    if valid_acc > best_acc:
                        ...
                        best_acc = valid_acc
                        saver.save(sess, 'model5/{}'.format(cur_model_name))
