...
def SVHNmodel(input_x, input_y, label_length, is_training=False, drop_rate=0.5):
    ...
    with tf.variable_scope('hidden_layer_2'):
        conv2 = tf.layers.conv2d(inputs=layer1, filters=64, kernel_size=[5, 5], padding='same')
    ...
    with tf.variable_scope('hidden_layer_3'):
        conv3 = tf.layers.conv2d(inputs=layer2, filters=128, kernel_size=[5, 5], padding='same')
    ...
    with tf.variable_scope('hidden_layer_4'):
        conv4 = tf.layers.conv2d(inputs=layer3, filters=160, kernel_size=[5, 5], padding='same')
    ...
    with tf.variable_scope('hidden_layer_5'):
        conv5 = tf.layers.conv2d(inputs=layer4, filters=192, kernel_size=[5, 5], padding='same')
    ...
    with tf.variable_scope('hidden_layer_6'):
        conv6 = tf.layers.conv2d(inputs=layer5, filters=192, kernel_size=[5, 5], padding='same')
    ...
    with tf.variable_scope('hidden_layer_7'):
        conv7 = tf.layers.conv2d(inputs=layer6, filters=192, kernel_size=[5, 5], padding='same')
    ...
    with tf.variable_scope('hidden_layer_8'):
        conv8 = tf.layers.conv2d(inputs=layer7, filters=192, kernel_size=[5, 5], padding='same')
    ...
    flatten = tf.reshape(layer8, [-1, 3072]) #3072 get from the paper
    with tf.variable_scope('densely_hidden_layer_1'):
        dense1 = tf.layers.dense(inputs=flatten, units=3072, activation=tf.nn.relu)
        layer9 = dense1
...
def training(x_train, train_label, train_label_length,
    ...
    with tf.Session() as sess:
        ...
        writer = tf.summary.FileWriter("log2/{}".format(cur_model_name), sess.graph)
            ...
            for itr in range(iters):
                ...
                if iter_total % 500 == 0:
                    ...
                    if valid_acc > best_acc:
                        ...
                        best_acc = valid_acc
                        saver.save(sess, 'model2/{}'.format(cur_model_name))
